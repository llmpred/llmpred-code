...
def read_data(path_in, path_max, dataset, dataset_max, size, feat, alpha, model, freq):
    file_name = 'run_individual_feat_' + dataset + '_' + model + '_' + size + '_' + size + '_1_' + str(
        alpha) + '_1_' + feat + '_200_' + freq
...
def denormalize(df, df_max, feat):
    prompt_ids = df['prompt_id'].unique()

    list_df = []
    for p_id in prompt_ids:
        df_sub = df[(df['prompt_id'] == p_id)]
        constant_max = df_max.at[int(p_id), 'F' + str(int(feat) + 1)]

        df_sub['pred_1'] = df_sub['pred_1'] * constant_max
        df_sub['gt_1'] = df_sub['gt_1'] * constant_max
    ...
    for pr in unique_prompts:
        ...
        for ba in unique_batches:
            ...
            if df_h_pr_ba.shape[0] > 0 and df_l_pr_ba.shape[0] > 0:
                gt_l = df_l_pr_ba['gt_1'].values
                gt_h = df_h_pr_ba['gt_1'].values
                gt_l, gt_h = get_min_len(gt_l, gt_h)
                gt_c = gt_l + gt_h

                pred_l = df_l_pr_ba['pred_1'].values
                pred_h = df_h_pr_ba['pred_1'].values
                ...
                data = np.concatenate((pred_c.reshape([-1, 1]),
                                       gt_c.reshape([-1, 1])),
                                      axis=1)
                df_new = pd.DataFrame(columns=['pred', 'gt'],
                                      data=data)
                df_new['pompt_id'] = pr
                df_new['batch_id'] = ba
...
if __name__ == '__main__':
    ...
    path_in_mlp = path_curr + '/data_uni/data_5_nn_out'
    path_in_gauss = path_curr + '/data_uni/data_6_gaussian_tranform'
    path_max = path_curr + '/data_uni/data_1_input_data'
    path_out = path_curr + '/data_uni/data_7_combine'
        ...
        for s, size in enumerate(['48', '96']):
            for model in models:
                for f, feat in enumerate(feats):
                    ...
                    df_data_l = denormalize(feat_data_l, max_data_l, feat)
                    df_data_h = denormalize(feat_data_h, max_data_h, feat)

                    # combine two dfs together
                    final_df = combine_taces(df_data_l, df_data_h)
                    file_name = file_name[:-4] + 'comb'
                    final_df.to_csv(path_out + '/' + file_name + '.csv',
                                    index=False)
