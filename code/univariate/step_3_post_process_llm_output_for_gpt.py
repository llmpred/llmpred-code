...
def check_integer_part(val, expected_value):
    ...
    # # Create the new array with the expected integer part
    # adjusted_array = expected_value + decimal_part
...
def get_fetures_data(path_llm_out_text, path_max_file_csv, limit, num_feat, spec_feat):
    ...
    while c < len(lines):
        ...
        if 'consider' in l:
            ...
            is_bad_segment = False
            sample = 0
            while (not 'print prompt' in l) and (not 'consider' in l):
                sample += 1
                l = lines[c]
                list_temp = []
                list_temp_descaled = []
                bad_segment_detected = False
                while len(l) > 0 and l != '\n' and (not 'print' in l) and (not 'consider' in l):
                    l = l[:-1]
                    ...
                    if c < len(lines):
                        ...
                        bad_segment_detected = is_bad_segment
                        break
...
# csv files
path_curr = Path.cwd()
path_max_val = path_curr.parent.__str__() + '/data_uni/data_1_input_data'
path_llm_out = path_curr.parent.__str__() + '/data_uni/data_2_output_data'
path_llm_post_processed = path_curr.parent.__str__() + '/data_uni/data_3_llm_post_processed'
