...
from transformers import LlamaForCausalLM, AutoModelForCausalLM, LlamaTokenizer, AutoTokenizer
...
class Model(nn.Module):
    ...
    def forward(self, prompt, bs, max_tokens):
        ...
        generate_ids = self.llama.generate(**batch,
                                           ...
                                           # no_repeat_ngram_size=2,  # Avoids repeating bigrams (pairs of words)
                                           # early_stopping=True  # Stops generation when quality drops
                                           # alpha=0.99,
                                           # beta = 0.3
                                           )
...
def get_train_test_prompts_range_of_feat(prompt, train_len, test_len):
    # front_prompt = 'consider the vertical and horizontal distribution patterns of the data and predict the next few lines'
    # front_prompt = 'consider the vertical and horizontal distribution. Value ranges of columns SHOULD be first: 0-1, second: 1-2 etc.) Predict the nex few lines. ONLY provide numerical values'
...
# select the range of prompts rather than one prompt
